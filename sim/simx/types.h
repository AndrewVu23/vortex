...
inline std::ostream &operator<<(std::ostream &os, const VsetType &type) {
  switch (type) {
  case VsetType::VSETVLI:
    os << "VSETVLI";
    break;
  case VsetType::VSETIVLI:
    os << "VSETIVLI";
    break;
  case VsetType::VSETVL:
    os << "VSETVL";
    break;
  default:
    assert(false);
...
using OpType = std::variant<
    AluType, BrType, MdvType, LsuType, AmoType, FpuType, CsrType, VoteType, ShflType, WctlType
#ifdef EXT_V_ENABLE
    ,
    VsetType, VlsType, VopType
#endif
#ifdef EXT_TCU_ENABLE
    ,
    TcuType
...
using IntrArgs = std::variant<
    IntrAluArgs, IntrBrArgs, IntrMdvArgs, IntrLsuArgs, IntrAmoArgs, IntrFpuArgs, IntrCsrArgs, IntrWctlArgs
#ifdef EXT_V_ENABLE
    ,
    IntrVsetArgs, IntrVlsArgs, IntrVopArgs
#endif
#ifdef EXT_TCU_ENABLE
    ,
    IntrTcuArgs
...
public:
  MatrixArbiter(uint32_t size)
      : size_(size), priority_matrix_(size, std::vector<bool>(size)) {
    this->reset();
  }
...
public:
  ...
  TFifo(const SimContext &ctx, const char *name, uint32_t delay, uint32_t capacity = 0)
      : SimObject<TFifo<T>>(ctx, name), bus_(this, capacity), delay_(delay) {
  }
...
public:
  ...
  TxArbiter(
      ...
      uint32_t delay = 1) : SimObject<TxArbiter<Type>>(ctx, name), Inputs(num_inputs, this), Outputs(num_outputs, this), delay_(delay), lg2_num_reqs_(log2ceil(num_inputs / num_outputs)), arbiters_(num_outputs, {type, 1u << lg2_num_reqs_}) {
    assert(delay != 0);
...
public:
  ...
  TxCrossBar(
      ...
      uint32_t delay = 1)
      : SimObject<TxCrossBar<Type>>(ctx, name), Inputs(num_inputs, this), Outputs(num_outputs, this), delay_(delay), lg2_inputs_(log2ceil(num_inputs)), lg2_outputs_(log2ceil(num_outputs)), output_sel_(output_sel), collisions_(0) {
    assert(delay != 0);
    assert(num_inputs <= 64);
    assert(num_outputs <= 64);
...
public:
  ...
  TxRxArbiter(
      ...
      : SimObject<TxRxArbiter<Req, Rsp>>(ctx, name), ReqIn(num_inputs, this), RspIn(num_inputs, this), ReqOut(num_outputs, this), RspOut(num_outputs, this), arbiter_(nullptr), rsp_delay_(rsp_delay), lg2_num_reqs_(log2ceil(num_inputs / num_outputs)) {
    if (num_inputs != num_outputs) {
      // allocate arbiter
...
public:
  ...
  TxRxCrossBar(
      ...
      uint32_t req_delay = 1,
      uint32_t rsp_delay = 1)
      : SimObject<TxRxCrossBar<Req, Rsp>>(ctx, name), ReqIn(num_inputs, this), RspIn(num_inputs, this), ReqOut(num_outputs, this), RspOut(num_outputs, this), crossbar_(nullptr), arbiter_(type, num_outputs), rsp_delay_(rsp_delay), lg2_inputs_(log2ceil(num_inputs)) {
