...
LsuUnit::LsuUnit(const SimContext &ctx, Core *core)
    : FuncUnit(ctx, core, "lsu-unit"), pending_loads_(0) {}
...
void LsuUnit::tick() {
  ...
  for (uint32_t iw = 0; iw < ISSUE_WIDTH; ++iw) {
    ...
    if (std::get_if<LsuType>(&trace->op_type)) {
      ...
      is_prefetch = (lsu_type == LsuType::PREFETCH);
      ...
      is_write = (vls_type == VlsType::VS || vls_type == VlsType::VSS || vls_type == VlsType::VSX);
    }
#endif // EXT_V_ENABLE
    else {
      std::abort();
    ...
    if (remain_addrs_ != 0) {
      ...
      lsu_req.is_prefetch = is_prefetch;
