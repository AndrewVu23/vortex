...
public:
  ...
  bool lookup(const bank_req_t &bank_req) {
    for (auto &entry : entries_) {
      ;
      if (entry.bank_req.type != bank_req_t::None && entry.bank_req.set_id == bank_req.set_id && entry.bank_req.addr_tag == bank_req.addr_tag) {
        return true;
      }
    }
    return false;
  ...
  mshr_entry_t &replay(uint32_t id) {
    ...
    for (auto &entry : entries_) {
      if (entry.bank_req.type == bank_req_t::Core && entry.bank_req.set_id == root_entry.bank_req.set_id && entry.bank_req.addr_tag == root_entry.bank_req.addr_tag) {
...
public:
  ...
  CacheBank(const SimContext &ctx,
            ...
            uint32_t bank_id)
      : SimObject<CacheBank>(ctx, name), core_req_port(this), core_rsp_port(this), mem_req_port(this), mem_rsp_port(this), config_(config), params_(params), bank_id_(bank_id), sets_(params.sets_per_bank, params.lines_per_set), mshr_(config.mshr_size), pipe_req_(TFifo<bank_req_t>::Create("", config.latency - 1)) {
    this->reset();
  }
...
private:
  void processInputs() {
    // process inputs in prioroty order
    do {
      bank_req_t bank_req;
      ...
      if (!this->core_req_port.empty()) {
        ...
        // check MSHR capacity
        if ((!core_req.write || config_.write_back) && (pending_mshr_size_ >= mshr_.capacity())) {
...
CacheSim::CacheSim(const SimContext &ctx, const char *name, const Config &config)
    : SimObject<CacheSim>(ctx, name), CoreReqPorts(config.num_inputs, this), CoreRspPorts(config.num_inputs, this), MemReqPorts(config.mem_ports, this), MemRspPorts(config.mem_ports, this), impl_(new Impl(this, config)) {}
